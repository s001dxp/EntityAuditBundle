...
class LogRevisionsListener implements EventSubscriber
{
	...
	private function saveRevisionEntityData($class, $entityData, $revType)
	{
		...
		foreach ($class->associationMappings AS $field => $assoc) {
			...
			if($data !== null && isset($assoc['id']) && $assoc['id'] === true){
				$data = $this->em->getReference($assoc['targetEntity'], $data);
			}
			...
			if($relatedId && count($relatedId) > 1){
				$idKeys = array_keys($relatedId);
				foreach($targetClass->identifier as $identifier){
					if(in_array($identifier, $idKeys)){
						if(($assoc['type'] & ClassMetadata::TO_ONE) > 0 && $assoc['isOwningSide']){
							$params[] = $relatedId[$identifier];
							if (isset($targetClass->fieldMappings[$identifier])) {
								$columnName = $targetClass->fieldMappings[$identifier]['columnName'];
							}
							else{
								$columnName = $targetClass->associationMappings[$identifier]['joinColumns'][0]['name'];
							}
							$types[] = $targetClass->getTypeOfColumn($columnName);
						}
					}
				}
			}
			...
			foreach ($assoc['sourceToTargetKeyColumns'] as $sourceColumn => $targetColumn) {
				...
				if ($data === null) {
					...
					if(($relatedId && count($relatedId) < 2) || false === $relatedId){
